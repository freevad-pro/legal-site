...
- **`POST /api/v1/auth/logout`** — удаляет текущую сессию из БД, очищает cookie.
- **`GET /api/v1/auth/me`** — всегда отдаёт 200 с телом `{login: str | null}`. `null` — анонимный посетитель (нет cookie или сессия истекла), строка — текущий вошедший пользователь. Фронт ветвит шапку по значению, без перехвата ошибок; 401 зарезервирован для случаев, когда отсутствие сессии — это действительно ошибка (logout, `with_llm=true` без сессии).
...
- TTL сессии — **30 дней** (`SESSION_TTL_DAYS`), rolling: `last_seen_at` обновляется при каждом запросе под cookie; истёкшие сессии очищаются лениво при следующем обращении к auth-эндпоинту (без отдельного фон-потока).
