...
- **`secrets.token_urlsafe`** (stdlib) — генерация идентификатора сессии (256 бит энтропии), без сторонних либ
...
- **Форма входа на сайте**, не браузерный диалог. `POST /api/v1/auth/login {login, password}` → проверка bcrypt-хэша → создаётся серверная сессия, в ответ ставится HttpOnly Secure SameSite=Lax cookie с `session_id`.
- **`POST /api/v1/auth/logout`** — идемпотентный 204: удаляет сессию из БД (если cookie есть) и явно очищает cookie на клиенте; без cookie тоже 204. Фронту не нужно различать «уже вышли» и «успешно вышли».
- **`GET /api/v1/auth/me`** — всегда отдаёт 200 с телом `{login: str | null}`. `null` — анонимный посетитель (нет cookie или сессия истекла), строка — текущий вошедший пользователь. Фронт ветвит шапку по значению, без перехвата ошибок; 401 зарезервирован для случая, когда отсутствие сессии — это действительно ошибка: `POST /scans` с `with_llm=true` без сессии.
- Пользователи в SQLite: `users(login TEXT PRIMARY KEY, password_hash TEXT)`, пароли — bcrypt.
- Сессии в SQLite — таблица `sessions(session_id TEXT PRIMARY KEY, user_login TEXT, created_at, expires_at, last_seen_at)`. Хранение на сервере, не stateless JWT — можно отозвать; для 1–2 пользователей оверхеда нет.
- TTL сессии — **30 дней** (`SESSION_TTL_DAYS`), rolling: `last_seen_at` обновляется при каждом запросе под cookie. Истёкшие записи чистятся двумя путями: лениво в `login` (когда мы и так блокирующе ходим в БД) и фоновой таской раз в час (по образцу purge-цикла для scan registry), чтобы у долго не входящего пользователя БД не разрасталась.
- Регистрации нет. Создание/обновление пользователя — CLI: `uv run python tools/create_user.py <login>` (пароль запрашивается интерактивно, в БД попадает только хэш). Восстановления пароля по email, личного кабинета, смены пароля через UI — тоже нет.
- **Смена пароля автоматически инвалидирует все активные сессии** этого пользователя: CLI после `upsert` в `users` делает `DELETE FROM sessions WHERE user_login = ?` в той же транзакции. Это защита на случай, если пароль меняют из-за подозрения на утечку — продолжать пускать старые `session_id` было бы лицемерием.
- В MVP — 1–2 пользователя, заводятся вручную владельцем сервера.

**Какие эндпоинты под входом:**
- `/health` — публичный.
- `GET /api/v1/scans/{id}`, `/events`, `/report.pdf` — публичные. Идентификатор скана — UUID4, угадать невозможно; гонять авторизацию по этим эндпоинтам нет смысла.
- `POST /api/v1/scans {url, with_llm: bool = false}` — публичный по умолчанию. Если `with_llm=true` — требуется валидная сессия, иначе 401. Логика: детерминированные правила корпуса бесплатны для всех, семантические LLM-функции работают на платных внешних API, поэтому ограничены владельцем.
- `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/me` — собственно auth-эндпоинты.
...
- Регистрация пользователей, OAuth, JWT в localStorage, восстановление пароля по email, личный кабинет, смена пароля через UI — только форма входа + cookie-сессия на сервере + ручное создание пользователя через CLI
...
│   ├── db.py                      # SQLite: init, схема (users, sessions, llm_cache)
│   ├── auth.py                    # cookie-сессии + bcrypt; Depends для опционального и обязательного пользователя
│   ├── api/
│   │   ├── __init__.py
...
```
   Browser (HTTPS, cookie-сессия после входа)
...
│   /api/v1/auth/login            ◄── ставит cookie       │
│   /api/v1/auth/logout           ◄── чистит cookie       │
...
│   │   ├─ Checks registry (детерм. всегда +          │   │
│   │   │   LLM только при with_llm=true)             │   │
...
1. `POST /api/v1/scan {url, with_llm}` → если `with_llm=true`, проверяется cookie-сессия (нет валидной → 401); создаётся `ScanState` с зафиксированным флагом `with_llm`, async-таска ждёт семафор; клиенту возвращается `scan_id`.
...
4. Детерминированные check-функции работают всегда. Семантические LLM-функции вызываются только при `state.with_llm=true` (и доступном `LLMProvider`); иначе они полностью пропускаются, никаких finding'ов не публикуют. Сами вызовы LLM идут через SQLite-кэш (см. раздел «Работа с LLM»).
...
ScanState
 ├─ scan_id: UUID
 ├─ url: str
 ├─ with_llm: bool                       # зафиксирован при создании;
 │                                        # требует валидной сессии в момент POST
...
CREATE TABLE sessions (
    ...
    created_at     TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    expires_at     TIMESTAMP NOT NULL,       -- created_at + SESSION_TTL_DAYS, rolling
...
Структура простейшая — никаких FK кроме одного на `sessions.user_login`, никаких сложных индексов. Запросы — по `PRIMARY KEY` и точечный select по `expires_at` для ленивой очистки истёкших сессий (фон-потока нет — чистим при следующем обращении к auth-эндпоинту).
...
### Доступ к LLM-проверкам

LLM-функции из реестра check-функций вызываются engine'ом **только если** `ScanState.with_llm == True`. Флаг фиксируется при создании скана (`POST /api/v1/scan {url, with_llm}`) и требует валидной сессии в момент запроса; без сессии и с `with_llm=true` API возвращает 401 ещё до запуска engine.

Если флаг выключен (анонимный пользователь или явный отказ авторизованного):
- LLM-провайдер вообще не дёргается, токены не тратятся, в SSE не публикуются «inconclusive из-за выключенного LLM» — семантические check-функции просто пропускаются, как будто их нет в корпусе.
- В UI и PDF это явно отображается отдельным блоком («Расширенный анализ доступен после входа»), чтобы пользователь понимал — это не результат «всё ок», а сознательное ограничение бесплатной версии.

Это первая ступень монетизации: бесплатные детерминированные проверки покрывают ~65% корпуса для всех, расходы на LLM-вызовы ограничены кругом авторизованных владельцев.
...
1. Открывает URL приложения в браузере — никаких диалогов, попадает сразу на главную.
2. Видит инпут для URL, кнопку «Проверить» и под ней мелкую строку «Расширенный анализ (доступен после входа)» — тоггл выключен.
3. Может запустить базовую проверку прямо сейчас, бесплатно, без регистрации.
4. Если хочет расширенный анализ — нажимает «Войти» в шапке → попадает на форму, вводит логин/пароль (созданные владельцем заранее через `make user`).
5. После успешного входа возвращается на главную: в шапке вместо «Войти» — `<login>` + «Выйти», тоггл расширенного анализа стал активным. Cookie с `session_id` живёт до 30 дней (`SESSION_TTL_DAYS`) и продлевается при активности.

### 2. Основной сценарий — проверка сайта (happy path)

1. Вводит URL в свободной форме — приложение нормализует (см. ниже). Если пользователь авторизован и тоггл «Расширенный анализ» включён — это передастся как `with_llm=true`. Иначе уходит `with_llm=false` (или поле опущено). Нажимает «Проверить».
2. Фронт делает `POST /api/v1/scan {url, with_llm}` → получает `scan_id`, сразу открывает `GET /api/v1/scan/{id}/events` (SSE).
...
| Ввёл неправильный пароль | Форма логина показывает inline-ошибку «Неверный логин или пароль», поля остаются заполненными (кроме пароля), пользователь не покидает страницу — никакого браузерного диалога |
| Попытался запустить расширенный анализ без входа (`with_llm=true` без cookie) | API отвечает 401; фронт мягко предлагает «Войдите, чтобы включить расширенный анализ», базовая проверка остаётся доступной одним кликом |

### 6. Чего НЕТ в сценариях MVP

- Регистрации, восстановления пароля по email, смены пароля через UI, личного кабинета (только `make user`).
...
| `SESSION_COOKIE_NAME` | `legal_site_session` | Имя cookie, в котором хранится `session_id` |
| `SESSION_TTL_DAYS` | `30` | Срок жизни сессии; продлевается при каждой активности (rolling) |
| `SESSION_COOKIE_SECURE` | `true` | На проде `true` (только HTTPS); для локальной разработки на `http://127.0.0.1` ставится `false`, иначе браузер не сохранит cookie |
...
SESSION_COOKIE_NAME=legal_site_session
SESSION_TTL_DAYS=30
...
- **Уровни:** `INFO` — старт/конец скана, найденные нарушения, создание пользователя, успешный login/logout, чистка истёкших сессий; `WARNING` — ретраи, отказ модерации, попадание в таймаут отдельной check-функции, превышение бюджета, попытка login с неверным паролем (логируется только факт, без пароля и без полного login'а в открытом виде); `ERROR` — Playwright-таймаут на главной, провайдер не отвечает после ретрая, неперехваченное исключение; `DEBUG` — выключено по умолчанию, включается через `LOG_LEVEL=DEBUG`.
