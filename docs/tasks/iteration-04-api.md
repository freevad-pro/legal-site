...
### Решение по auth: Basic Auth как временное MVP-решение

Продуктовая модель проекта — «бесплатные детерминированные проверки доступны всем, LLM-анализ — только владельцу» — требует двухветочного контракта (`with_llm=false` без auth, `with_llm=true` под сессией) и формы входа на сайте, а не браузерного диалога.

В итерации 4 для скорости был использован сквозной HTTP Basic Auth поверх всех `/api/v1/scans/*`: это снимало вопрос аутентификации целиком и позволило сосредоточиться на API, SSE-прогрессе, нормализации URL и PDF-рендере. Переход к целевой модели — отдельной итерацией 5а ([tasks/iteration-05a-auth-refactor.md](iteration-05a-auth-refactor.md)): cookie-сессии в SQLite, форма `/login`, разделение `POST /scans` по флагу `with_llm`. Закрытая итерация 4 остаётся как исторический след — её код переписывается в 5а.
